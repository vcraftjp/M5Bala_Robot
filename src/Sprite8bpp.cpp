...
void Sprite8bpp::blendAlphaBitmap(int32_t x, int32_t y, int32_t w, int32_t h, const uint8_t *bitmap, int ratio, uint8_t brightness) {
	if (x < 0) { w += x; x = 0; }
...
void Sprite8bpp::flush(uint16_t *palette, int destY) {
	if (palette == NULL) {
		pushSprite(0, 0);
		return;
	}
...
//	inTransaction = true;
	...
	setAddrWindow(x, destY, x + w - 1, destY + h - 1);
	uint16_t lineBuf[w];
